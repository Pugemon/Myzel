...
internal sealed class YamlTextWriter(TextWriter writer) : IDisposable
{
    ...
    {
        if (_stateStack.TryPeek(out WriterState state))
        {
            switch (state)
            {
    ...
    public void WriteEndArray()
    {
        ...
        if (_stateStack.TryPeek(out WriterState state) && state == WriterState.Property) _stateStack.Pop();
    }
    ...
    {
        InternalWriteValue(value ? "true" : "false");
    }
    public void WriteValue(byte value)
    {
        InternalWriteValue(value.ToString());
    }
    ...
    {
        if (UnsafeChars.Any(c => name.IndexOf(c) > -1))
        {
            return "'" + name + "'";
        }
