...
namespace Myzel.GUI.Essentials.Helpers;
...
public static class PlatformHelper
{
    ...
    static PlatformHelper()
    {
        ...
        {
            PlatformHelper.Platform = RuntimeInformation.OSArchitecture switch
            {
                Architecture.X64 => PlatformId.WinX64,
                Architecture.Arm64 => PlatformId.WinArm64,
                _ => PlatformId.Unknown
            };
            PlatformHelper.ExecutableExtension = ".exe";
        }
        ...
        {
            PlatformHelper.Platform = PlatformId.Wasm;
        }
    }
    ...
    public static string? GetFullPath(string fileName)
    {
        ...
        return values == null ? fileName : values.Split(Path.PathSeparator).Select(path => Path.Combine(path, fileName)).FirstOrDefault(File.Exists);
    ...
    public static void OpenHyperLink(string link)
    {
        try
        {
            ProcessStartInfo ps = new ProcessStartInfo(link)
            {
